...
def test_notifier_dedupe_and_rate_limit():
    ...
    notifier = Notifier(channels=[fake_channel], rate_limit=("1/sec", 1),
                        dedupe_window=2, retries=(0, 1.0))

    s = StringIO()
    log.configure(service="t", env="test", stream=s)

    # 1st EMERGENCY — must send
    log.emergency("inv_broken", account_id=1, _notify=notifier)
    # same payload in <2s — dedupe (does not send)
    log.emergency("inv_broken", account_id=1, _notify=notifier)

    time.sleep(1.1)
    # different payload (account_id changes) — must send (rate-limit released)
